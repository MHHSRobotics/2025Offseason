...
public class SwerveCommands {
    ...
    public Command stop(){
        return new InstantCommand(()->swerve.stop(),swerve);
    }
    ...
    public Command drive(DoubleSupplier dx,DoubleSupplier dy,DoubleSupplier omega,BooleanSupplier fieldRelative){
        return Commands.run(()->{
            ...
            double scale=Math.pow(MathUtil.applyDeadband(radius, Swerve.Constants.moveDeadband),Swerve.Constants.movePow);
            ...
            // Apply the deadband to the absolute value of rotation and take it to the power of turnPow for smoother control
            double rotationScale=Math.pow(MathUtil.applyDeadband(Math.abs(rotation), Swerve.Constants.turnDeadband),Swerve.Constants.turnPow);
