...
public class Swerve extends SubsystemBase {
    public static class Constants {
        // Deadband in the move controls, how far away the stick needs to be from the origin before the bot starts moving
        public static final double moveDeadband=0.1;

        // Deadband in the turn controls, how far away the stick needs to be from the origin before the bot starts turning
        public static final double turnDeadband=0.1;

        // Exponent applied to the move to make small adjustments easier. E.g. instead of moving 0.5 when the stick is at halfway, the bot would move 0.5^2 = 0.25 with movePow 2.
    ...
    // This is the pose estimator for the swerve drive. It looks at odometry and vision measurements and gives a best estimate for the current location of the bot.
    private SwerveDrivePoseEstimator estimator =
            new SwerveDrivePoseEstimator(kinematics, gyroAngle, getModulePositions(), new Pose2d());
    ...
    // Helper method to create a line connecting two swerve modules with the given indicesin the visualization
    private void connect(int firstIndex,int secondIndex){
        ...
        roots[firstIndex].append(new LoggedMechanismLigament2d("", diff.getNorm(), diff.getAngle().getRadians(), 5, new Color8Bit(Color.kWhite)));
    }
    ...
    private void initializeRoots(){
        ...
        for(int i=0;i<4;i++){
            LoggedMechanismRoot2d newRoot=mech.getRoot("Root"+i, 1.5+moduleTranslations[i].getX(), 1.5+moduleTranslations[i].getY());
            roots[i]=newRoot;
            // Create the line representing speed of the module
            speeds[i]=newRoot.append(new LoggedMechanismLigament2d("Speed"+i, 0, 0, 5,new Color8Bit(Color.kRed)));
        }
    ...
    // Runs the swerve module with the given dx, dy, and omega. If fieldRelative is true then its movement will be relative to the current alliance's field.
    public void runSpeeds(double dx,double dy,double omega,boolean fieldRelative){
        ChassisSpeeds speeds=new ChassisSpeeds(dx*getMaxLinearSpeedMetersPerSec(), dy*getMaxLinearSpeedMetersPerSec(), omega*getMaxAngularSpeedRadPerSec());
        if(fieldRelative){
    ...
    // Locks the swerve modules into an X position to prevent the bot from being pushed. When runVelocity() is called again the modules will return to their regular positions.
    public void lock() {
