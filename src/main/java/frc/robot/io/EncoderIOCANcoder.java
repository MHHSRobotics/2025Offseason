 ...
 * - Total offset = (MagnetOffset * 2π / encoderRatio) + extraOffset (both in mechanism units)
...
public class EncoderIOCANcoder extends EncoderIO {
    ...
    public void update() {
        ...
        // 2. Convert to rotations and apply gear ratio: enc_rot / encoderRatio
        // 3. Convert to radians and subtract software offset to get final mechanism position
        inputs.positionRad =
    ...
    /**
     * Sets the offset of the encoder (mechanism units).
     *
     * OFFSET SPLITTING ALGORITHM:
     * 1. Convert mechanism offset to rotations
     ...
     *
     * @param mechOffset Desired offset (mechanism units)
     */
    @Override
    ...
    /**
     * Sets the simulated mechanism position (mechanism units).
    ...
    public void setMechPosition(double position) {
        ...
        // 2. Convert to rotations: (position + extraOffset) / 2π
        // 3. Apply gear ratio: ((position + extraOffset) / 2π) * encoderRatio
        // 4. Add hardware offset: final + MagnetOffset
        double encoderPos = Units.radiansToRotations(position + extraOffset) * encoderRatio
    ...
    /**
     * Sets the simulated mechanism velocity (mechanism units/s).
